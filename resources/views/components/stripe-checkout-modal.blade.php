{{-- Stripe Embedded Checkout slide-over panel — renders Stripe's checkout form in an iframe.
     Slides in from the right for better UX with tall checkout forms.
...
>
	...
	>
		...
		<div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
			...
			<button @click="closeCheckout()" class="cursor-pointer rounded-md p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600">
		...
		<div class="flex-1 overflow-y-auto">
			...
			{{-- Error state --}}
			<div x-show="errorMessage" class="px-6 py-20 text-center">
			...
			<div x-show="!loading && !errorMessage" id="stripe-checkout-container"></div>
		</div>
	</div>
</div>
...
<script>
	document.addEventListener("alpine:init", () => {
		Alpine.data("stripeCheckoutPanel", () => ({
			visible: false,
			loading: false,
			errorMessage: null,
			stripeInstance: null,
