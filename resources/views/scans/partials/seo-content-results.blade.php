...
@php
	...
	$defaultSeoSection = $seoGrouped->keys()->first() ?? "";
@endphp
		...
		<div class="flex flex-row-reverse min-h-[500px] gap-12">
			...
			<div class="w-72 shrink-0">
				<nav class="sticky top-6 max-h-[calc(100vh-3rem)] overflow-y-auto pb-3">
					@foreach($seoGrouped as $categoryName => $categoryModules)
						@php
						...
						@endphp
						<button
							@click="seoSection = '{{ $categoryName }}'"
							:class="seoSection === '{{ $categoryName }}' ? 'bg-gray-300 text-text-primary font-medium' : 'text-text-secondary hover:bg-gray-50 hover:text-text-primary'"
						...
						>
							...
							<span class="shrink-0 rounded-full {{ $scoreColor }} px-2 py-0.5 text-[11px] font-medium transition-colors">{{ $categoryPassed }}/{{ $categoryTotal }}</span>
						</button>
					@endforeach
				</nav>
			</div>
			...
			<div class="flex-1 min-w-0 overflow-visible">
				@foreach($seoGrouped as $categoryName => $moduleResults)
					<div x-show="seoSection === '{{ $categoryName }}'" x-cloak>
						<div class="mb-6">
